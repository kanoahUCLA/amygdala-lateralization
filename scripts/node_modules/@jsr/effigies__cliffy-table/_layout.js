...
/** Table layout renderer. */ export class TableLayout {
   ...
   */ createLayout() {
    ...
    /* Try to get the total table width within a maximum */ const totalWidth = sum(width);
    const maxAllowable = this.options.maxTableWidth - sum(padding.filter((x)=>x));
    if (totalWidth > maxAllowable && this.options.colRigidity != 1) {
      const rigidity = width.map((_w, i)=>Array.isArray(this.options.colRigidity) ? this.options.colRigidity[i] : this.options.colRigidity);
      ...
      if (slack > 0) {
        ...
        for(let colIndex = 0; colIndex < width.length; colIndex++){
          if (rigidity[colIndex] < 1) {
            ...
            const minColWidth = column?.getMinWidth() ?? (Array.isArray(this.options.minColWidth) ? this.options.minColWidth[colIndex] : this.options.minColWidth);
            const maxColWidth = column?.getMaxWidth() ?? (Array.isArray(this.options.maxColWidth) ? this.options.maxColWidth[colIndex] : this.options.maxColWidth);
            width[colIndex] = Math.min(maxColWidth, Math.max(minColWidth, Math.floor(width[colIndex] * flexFactor)));
          }
        }
      }
    }
