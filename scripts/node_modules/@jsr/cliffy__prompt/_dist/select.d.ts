...
/** Select prompt options. */ export interface SelectOptions<TValue> extends GenericListOptions<TValue, TValue, TValue> {
  /** Keymap to assign key names to prompt actions. */ keys?: SelectKeys;
  /** An array of child options. */ options: Array<Extract<WidenType<TValue>, string | number> | SelectOption<TValue> | SelectOptionGroup<TValue> | GenericListSeparatorOption>;
}
/** Select prompt settings. */ export interface SelectSettings<TValue> extends GenericListSettings<TValue, TValue, TValue, SelectOptionSettings<TValue>, SelectOptionGroupSettings<TValue>> {
  keys: SelectKeys;
}
/** Select option options. */ export type SelectOption<TValue> = GenericListOption<TValue>;
/** Select option group options. */ export type SelectOptionGroup<TValue> = GenericListOptionGroup<TValue, GenericListOption<TValue>>;
/** Select option settings. */ export type SelectOptionSettings<TValue> = GenericListOptionSettings<TValue>;
/** Select option group settings. */ export type SelectOptionGroupSettings<TValue> = GenericListOptionGroupSettings<TValue, SelectOptionSettings<TValue>>;
/** Select prompt keymap. */ export type SelectKeys = GenericListKeys;
/**
 ...
 */ export declare class Select<TValue> extends GenericList<TValue, TValue, TValue, SelectOptionSettings<TValue>, SelectOptionGroupSettings<TValue>> {
  ...
  /** Execute the prompt with provided options. */ public static prompt<TValue>(options: SelectOptions<TValue>): Promise<WidenType<TValue>>;
  /**
  ...
  public override getDefaultSettings(options: SelectOptions<TValue>): SelectSettings<TValue>;
  /** Map string option values to options and set option defaults. */ protected mapOptions(promptOptions: SelectOptions<TValue>, options: Array<Extract<TValue, string | number> | Extract<WidenType<TValue>, string | number> | SelectOption<TValue> | SelectOptionGroup<TValue> | GenericListSeparatorOption>): Array<SelectOptionSettings<TValue> | SelectOptionGroupSettings<TValue>>;
  protected override input(): string;
  protected override submit(): Promise<void>;
  /** Get value of selected option. */ protected getValue(): TValue;
  /**
