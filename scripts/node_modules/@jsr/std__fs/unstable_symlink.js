 ...
 */ export async function symlink(oldpath, newpath, options) {
  if (isDeno) {
    return Deno.symlink(oldpath, newpath, options);
  } else {
    try {
      return await getNodeFs().promises.symlink(oldpath, newpath, options?.type);
    } catch (error) {
 ...
 * @param newpath The path of the symbolic link.
 * @param options Options when creating a symbolic link.
 */ export function symlinkSync(oldpath, newpath, options) {
  if (isDeno) {
...
//# sourceMappingURL=unstable_symlink.js.map