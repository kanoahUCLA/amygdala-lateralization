import { Table } from "@jsr/effigies__cliffy-table";
import { bold, brightBlue, brightMagenta, dim, getColorEnabled, green, italic, red, setColorEnabled, yellow } from "@jsr/std__fmt/colors";
...
/** Help text generator. */ export class HelpGenerator {
  ...
  generateDescription() {
    ...
    ]).indent(this.indent).maxColWidth(this.width - this.indent).colRigidity(0).padding(1).toString() + "\n";
  }
  generateOptions() {
  ...
  generateOptionGroup(group) {
    ...
    if (hasTypeDefinitions) {
      return this.label(group.name ?? "Options") + Table.from([
        ...
        1,
        2
      ]).indent(this.indent).maxTableWidth(this.width - this.indent).colRigidity([
        1,
        1,
        1,
        0,
  ...
  generateCommands() {
    ...
    ]).maxTableWidth(this.width - this.indent).colRigidity([
      1,
      1,
      0
    ]).maxColWidth([
  ...
  generateExamples() {
    ...
    return this.label("Examples") + Table.from(examples.map((example)=>[
      ...
      ])).padding(1).indent(this.indent).maxTableWidth(this.width - this.indent).toString() + "\n";
  }
...
//# sourceMappingURL=_help_generator.js.map