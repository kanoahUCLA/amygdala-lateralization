 ...
 */ export function toTitleCase(input, options) {
  ...
  for (const s of segments){
    if (s.isWordLike) {
      out += !exclude(s, i++, words) ? titleCaseSegment(s.segment, opts) : opts.trailingCase === "lower" ? s.segment.toLocaleLowerCase(opts.locale) : s.segment;
    } else {
      out += s.segment;
    }
  }
...
function excludeStopWords(stopWords) {
...
function toExcludeFilter(exclude) {
  ...
  const filters = exclude.map((x)=>typeof x === "function" ? x : excludeStopWords(x));
  return (s, i, w)=>{
...
function isStrings(x) {
  return typeof x[0] === "string";
}
//# sourceMappingURL=unstable_to_title_case.js.map