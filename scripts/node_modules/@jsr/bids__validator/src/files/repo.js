 ...
 */ import { dirname, join, parse, SEPARATOR_PATTERN } from "@jsr/std__path";
import { default as git } from "isomorphic-git";
import { createMD5 } from "hash-wasm";
const textDecoder = new TextDecoder('utf-8');
export const annexKeyRegex = /^(?<hashname>[A-Z0-9]+)-s(?<size>\d+)--(?<digest>[0-9a-fA-F]+)(?<ext>\.[\w\-. ]*)?/;
export const rmetLineRegex = /^(?<timestamp>\d+(\.\d+)?)s (?<uuid>[0-9a-fA-F-]+):V \+(?<version>[^#]+)#(?<path>.+)/;
export async function readAnnexPath(filepath, options) {
 ...
 */ export async function hashDirLower(annexKey) {
 ...
 */ async function readRmet(key, options) {
  ...
  for (const line of rmet.split('\n')){
    ...
    if (match) {
      ret[match.groups.uuid] = match.groups;
    }
  }
  return ret;
}
 ...
 *    publicurl
 *    timestamp
 */ export async function readRemotes(options) {
 ...
 */ export async function resolveAnnexedFile(path, remote, options) {
