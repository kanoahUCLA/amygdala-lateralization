import { readEntities } from "../schema/entities.js";
 ...
 */ export function* walkBack(source, inherit = true, targetExtensions = [
  '.json'
], targetSuffix, targetEntities) {
  ...
  while(fileTree){
    const candidates = fileTree.files.filter((file)=>{
      ...
      return targetExtensions.includes(extension) && suffix === targetSuffix && Object.keys(entities).every((entity)=>entities[entity] === sourceParts.entities[entity] || targetEntities?.includes(entity));
    });
    if (candidates.length > 1) {
      const exactMatch = candidates.find((file)=>{
        const { entities } = readEntities(file.name);
        return [
          ...Object.keys(sourceParts.entities),
          ...targetEntities ?? []
        ].every((entity)=>entities[entity] === sourceParts.entities[entity]);
      });
      if (exactMatch) {
...
export async function readSidecars(source) {
  const ret = new Map();
