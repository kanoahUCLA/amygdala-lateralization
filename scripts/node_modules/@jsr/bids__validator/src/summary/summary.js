...
export const modalityPrettyLookup = {
  mri: 'MRI',
  pet: 'PET',
  meg: 'MEG',
  eeg: 'EEG',
...
export function computeModalities(modalities) {
  ...
  return sortedModalities.map((mod)=>mod in modalityPrettyLookup ? modalityPrettyLookup[mod] : mod);
}
function computeSecondaryModalities(secondary) {
  const nonZeroSecondary = Object.keys(secondary).filter((a)=>secondary[a] !== 0);
  const sortedSecondary = nonZeroSecondary.sort((a, b)=>secondary[b] - secondary[a]);
  ...
  async update(context) {
    ...
    if (context.extension === '.json') {
      if (typeof context.json === 'object' && 'TaskName' in context.json) {
        this.tasks.add(context.json.TaskName);
      }
    }
    if (context.modality) {
