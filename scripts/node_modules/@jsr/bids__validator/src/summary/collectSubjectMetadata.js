 ...
 */ export const collectSubjectMetadata = (participantsTsvContent)=>{
  ...
  const headers = snakeCaseHeaders.map((header)=>header === 'participant_id' ? PARTICIPANT_ID : header);
  const targetKeys = [
    PARTICIPANT_ID,
    'age',
    'sex',
  ...
  if (participantIdKey === undefined) return [];
  else {
    return subjectData.map((data)=>{
      ...
      data[participantIdKey.index] = data[participantIdKey.index].replace(/^sub-/, '');
      // make age an integer
      if (ageKey) {
      ...
      return data;
    }).map((data)=>//extract all target metadata for each subject
      targetKeys.reduce((subject, { key, index })=>({
          ...subject,
          [key]: data[index]
        }), {}));
  }
};
//# sourceMappingURL=collectSubjectMetadata.js.map