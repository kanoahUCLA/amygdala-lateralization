 ...
 */ // @ts-nocheck
import { globToRegExp } from "@jsr/std__path";
import { lookupEntityLiteral } from "./filenameValidate.js";
const CHECKS = [
  findRuleMatches,
  hasMatch,
  cleanContext
...
export async function findDirRuleMatches(schema, context) {
  ...
  loop: for (const key of Object.keys(directoryRule)){
    ...
    if ('entity' in node) {
      ...
      if (entityDef && 'name' in entityDef && context.file.name.startsWith(`${entityDef['name']}-`)) {
        context.filenameRules.push(path);
        break;
      }
    }
...
function findRuleMatches(schema, context) {
  ...
  Object.keys(schema[schemaPath]).map((key)=>{
    if (key == 'deriv' && context.dataset.dataset_description.DatasetType != 'derivative') {
      return;
    }
    const path = `${schemaPath}.${key}`;
 ...
 */ export function _findRuleMatches(node, path, context) {
  if (`/${node.path}` === context.path || node.stem && matchStemRule(node, context) || 'suffixes' in node && node.suffixes.includes(context.suffix)) {
    context.filenameRules.push(path);
    return;
  }
...
function matchStemRule(node, context) {
 ...
 */ function entitiesExtensionsInRule(schema, context, path) {
  ...
  const ruleEntities = rule.entities ? Object.keys(rule.entities).map((key)=>lookupEntityLiteral(key, schema)) : [];
 ...
 */ function cleanContext(schema, context) {
  ...
  filenameParts.map((part)=>{
    if (rules.every((rule)=>!rule[part[0]] || Object.keys(rule[part[0]]).length === 0)) {
      context[part[1]] = part[2];
    }
  });
}
//# sourceMappingURL=filenameIdentify.js.map