...
import { fileListToTree } from "./files/browser.js";
import { FileIgnoreRules } from "./files/ignore.js";
import { resolve } from "@jsr/std__path";
import { validate } from "./validators/bids.js";
import { consoleFormat, resultToJSONStr } from "./utils/output.js";
 ...
 */ export async function main() {
  ...
  const prune = options.prune ? new FileIgnoreRules([
    'derivatives',
    'sourcedata',
    'code'
  ], false) : undefined;
  const tree = await readFileTree(absolutePath, prune, options.preferredRemote);
  const config = options.config ? JSON.parse(Deno.readTextFileSync(options.config)) : {};
