import { validate } from "@jsr/bids__validator/main";
import { validateCommand } from "@jsr/bids__validator/options";
import { readFileTree } from "@jsr/bids__validator/files";
import { consoleFormat } from "@jsr/bids__validator/output";
import { logger } from "../logger.js";
import { Confirm, prompt } from "@jsr/cliffy__prompt";
import { join, relative, resolve } from "@jsr/std__path";
import { walk } from "@jsr/std__fs/walk";
import { getRepoAccess } from "./git-credential.js";
import { readConfig } from "../config.js";
import { createDatasetAffirmed } from "./create-dataset.js";
import { CreateDatasetAffirmedError } from "../error.js";
import validatorConfig from "../validator-config.json" with {
  type: "json"
};
async function getRepoDir(url) {
 ...
 */ export async function addGitFiles(worker, dataset_directory_abs) {
  // Upload all files
  for await (const walkEntry of walk(dataset_directory_abs, {
    includeDirs: false,
    includeSymlinks: true
  })){
    const relativePath = relative(dataset_directory_abs, walkEntry.path);
    if (relativePath === ".bidsignore" || !relativePath.startsWith(".")) {
      worker.postMessage({
        "command": "add",
        "path": walkEntry.path,
...
export async function uploadAction(options, dataset_directory) {
  ...
  logger.info(`upload ${dataset_directory} resolved to ${dataset_directory_abs}`);
  ...
  const schemaResult = await validate(await readFileTree(dataset_directory_abs), options, validatorConfig);
 ...
 */ export const upload = validateCommand.name("upload").description("Upload a dataset to OpenNeuro").option("--json", "Hidden for upload usage", {
  hidden: true,
  override: true
}).option("--filenameMode", "Hidden for upload usage", {
  hidden: true,
  override: true
}).option("-d, --dataset [dataset:string]", "Specify an existing dataset to update.", {
  conflicts: [
    "new"
  ]
...
}).option("--delete", "Similar to rsync --delete, remove files in the remote dataset that are not present in the local dataset.", {
  depends: [
    "dataset"
  ]
}).option("--affirmDefaced", "All structural scans have been defaced, obscuring any tissue on or near the face that could potentially be used to reconstruct the facial structure.", {
  default: false
}).option("--affirmConsent", "I have explicit participant consent and ethical authorization to publish structural scans without defacing.", {
  default: false
}).action(uploadAction);
