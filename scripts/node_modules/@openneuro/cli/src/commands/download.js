...
import { getLatestSnapshotVersion } from "../graphq.js";
export const download = new Command().name("download").description("Download a dataset from OpenNeuro").arguments("<accession_number> <download_directory>").option("-d, --draft", "Download a draft instead of the latest version snapshot.", {
  conflicts: [
    "version"
  ]
}).option("-v, --version <version:string>", "Download a specific version.", {
  conflicts: [
    "draft"
  ]
}).action(downloadAction);
export async function downloadAction(options, accession_number, download_directory) {
  ...
  if (options.version) {
    ...
    if (version.length === 40 /* sha1 */  || version.length === 64 /* sha256 */ ) {
